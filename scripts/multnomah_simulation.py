...
def main():
    new_dir = ("multnomah_simulation_results")
    check_dir = os.path.isdir("multnomah_simulation_results")
    if not check_dir:
            os.makedirs(new_dir)
    model = odyn.OpinionNetworkModel(probabilities = [1 - .24, .24],
    ...
    model.belief_df.to_csv("multnomah_simulation_results/belief_df.csv")
    ...
    for left_reach in [0,.25,.5,.75,1]:
        for right_reach in [0,.25,.5,.75,1]:
            ...
            sim.dynamic_belief_df.to_csv(f"multnomah_simulation_results/left_reach_{l:03}_right_reach_{r:03}_dynamic_belief_df.csv")
